...
-   If your application needs to access large datasets, the fastest way to do
    ...
    installed in `/home/ubuntu/ephemeral-hdfs`, and can be accessed using the
...
-   There is also a *persistent HDFS* instance in
    `/home/ubuntu/persistent-hdfs` that will keep data across cluster restarts.
...
-   Finally, if you get errors while running your application, look at the slave's logs
    for that application inside of the scheduler work directory (/home/ubuntu/spark/work). You can
    also view the status of the cluster using the web UI: `http://<master-hostname>:8080`.

## Configuration

You can edit `/home/ubuntu/spark/conf/spark-env.sh` on each machine to set Spark configuration options, such
...
then run `~/spark-ec2/copy-dir /home/ubuntu/spark/conf` to RSYNC it to all the workers.
...
drives and also creates files named `/home/ubuntu/spark-ec2/masters`, and `/home/ubuntu/spark-ec2/slaves`.
...
get a list of slave hostnames and `/home/ubuntu/spark-ec2/copy-dir` to sync a directory across machines.
